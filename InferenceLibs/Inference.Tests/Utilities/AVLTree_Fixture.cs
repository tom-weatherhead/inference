...
{
    [TestFixture]
    public class AVLTree_Fixture
    {
        private readonly IDictionary<int, string> avltree;

        public AVLTree_Fixture()
        {
            avltree = new AVLTree<int, string>();
        }
        ...
        {
            avltree.AddItems(numList.Select(n => new KeyValuePair<int, string>(n, n.ToString())));
        }

        private string GetStringOfValuesFromInOrderTraversal()
        {
            return string.Join(" ", avltree.Select(kvp => kvp.Value));
        }

        private void AssertKeyValueCorrespondence()
        {

            foreach (var kvp in avltree)
            {
        ...
        public void DeleteTest1()
        {
            ...
            sorted.Remove(10);
            successfulDeletion = avltree.Remove(10);
            ...
            successfulDeletion = avltree.Remove(10);    // Try to delete 10 again.
        ...
        public void FindTest()
        {
            ...
            bool keyWasFound = avltree.TryGetValue(4, out valueThatWasFound);
            ...
            keyWasFound = avltree.TryGetValue(4, out valueThatWasFound);

            Assert.IsTrue(keyWasFound);
            Assert.AreEqual("4", valueThatWasFound);

            // 2) Try to find an absent key in a non-empty tree.
            keyWasFound = avltree.TryGetValue(9, out valueThatWasFound);
        ...
        public void FindTest2FindAll()
        {
            ...
            {
                string valueThatWasFound;
                var keyWasFound = avltree.TryGetValue(n, out valueThatWasFound);
        ...
        [Test]
        public void DeleteAndFindPredecessorTest()
        {
            InsertRangeOfIntsAndStrings(new List<int>() { 2, 1, 3 });
            avltree.Remove(2);

            string valueThatWasFound;
            bool keyWasFound = avltree.TryGetValue(1, out valueThatWasFound);

            Assert.IsTrue(keyWasFound);
            Assert.AreEqual("1", valueThatWasFound);
        }
        ...
        public void ContainsNullValueTest()
        {
            ...
            Assert.IsFalse(list1.Contains(null));
            ...
            Assert.IsFalse(avltree.Contains(new KeyValuePair<int, string>(1, null)));
            Assert.IsTrue(avltree.Contains(new KeyValuePair<int, string>(1, "a")));
            Assert.IsTrue(avltree.Contains(new KeyValuePair<int, string>(2, null)));
            Assert.IsFalse(avltree.Contains(new KeyValuePair<int, string>(2, "b")));
        }
    }
}
