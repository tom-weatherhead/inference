...
using Inference.Interpreter.Smalltalk;
using Inference.Parser;
using NUnit.Framework;

using EvaluationException = Inference.Interpreter.EvaluationException;

namespace Inference.Tests.Interpreter.Smalltalk
{
    [TestFixture]
    public class Parser_Fixture
    {
        ...
        private readonly string falseValueAsString = SmalltalkGlobalInfo.FalseValueAsString;
        private readonly string trueValueAsString = SmalltalkGlobalInfo.TrueValueAsString;

        public Parser_Fixture()
        {
            tokenizer = TokenizerFactory.Create(GrammarSelector.Smalltalk);
            parser = ParserFactory.Create(ParserSelector.SLR1, GrammarSelector.Smalltalk);
        ...
        private ISmalltalkValue EvaluateToISmalltalkValue(string input)
        {
            var expr = GetParseResult(input) as ISmalltalkExpression;

            Assert.IsNotNull(expr);

            var smalltalkExpr = expr.Evaluate(null, globalInfo.ObjectInstance, null, globalInfo);

            Assert.IsNotNull(smalltalkExpr);

            return smalltalkExpr;
        }

        private string Evaluate(string input)
        {
            return EvaluateToISmalltalkValue(input).ToString();
        }
